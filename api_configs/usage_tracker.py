...
class APIConfigUsageTracker:
    ...
    def __new__(cls):
        ...
        if cls._instance is None:
            cls._instance = super(APIConfigUsageTracker, cls).__new__(cls)
            cls._instance._initialized = False
        return cls._instance
    
    def __init__(self, database_path: str = "./cache/api_usage"):
        """Initialize the tracker"""
        if getattr(self, "_initialized", False):
            return
            
        self.database_path = database_path
        self.logs_path = os.path.join(database_path, "logs")
        self.metrics_path = os.path.join(database_path, "metrics")
    ...
    def _save_usage_log(self, usage_log: APIConfigUsageLog) -> None:
        ...
        file_path = os.path.join(self.logs_path, f"{usage_log.id}.json")
    ...
    def _update_metrics(self, usage_log: APIConfigUsageLog) -> None:
        ...
        try:
            ...
            logger.error(f"Error updating metrics for API config {usage_log.api_config_id}: {e}")
    
    def _save_metrics(self, metrics: APIConfigMetrics) -> None:
        """Save metrics to the filesystem"""
        file_path = os.path.join(self.metrics_path, f"{metrics.api_config_id}.json")
        with open(file_path, "w") as f:
    ...
    def get_metrics(self, api_config_id: str) -> Optional[APIConfigMetrics]:
        ...
        """
        file_path = os.path.join(self.metrics_path, f"{api_config_id}.json")
    ...
    def get_top_users(self, api_config_id: str, limit: int = 10) -> List[tuple]:
        ...
        sorted_users = sorted(metrics.user_frequency.items(), key=lambda x: x[1], reverse=True)
